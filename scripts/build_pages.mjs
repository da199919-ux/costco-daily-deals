...
const deals = rows.map((row) => {
  ...
  return {
    ...
    stockStatus: column["庫存狀態"] === undefined ? "狀態未知" : (row[column["庫存狀態"]] || "狀態未知"),
    url,
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:radial-gradient(circle at 85% 0,#13355c 0,transparent 32%),var(--navy);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang TC","Noto Sans TC",sans-serif;min-height:100vh}
    ...
    .stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:24px 0}.stat{background:#0d2748cc;border:1px solid var(--line);padding:18px;border-radius:18px}.stat strong{display:block;font-size:1.8rem}.stat span{color:var(--muted);font-size:.86rem}
    ...
    .grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.card{display:flex;flex-direction:column;min-height:310px;background:linear-gradient(145deg,var(--panel2),var(--panel));border:1px solid var(--line);border-radius:20px;padding:12px 18px 18px;color:var(--text);transition:transform .18s,border-color .18s;overflow:hidden}.card:hover{transform:translateY(-3px);border-color:var(--cyan)}.card.out-of-stock{border-color:#e3193777}.product-link{color:inherit;text-decoration:none}.thumb{aspect-ratio:1/1;width:100%;height:auto;margin:0 0 14px;border-radius:14px;background:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden}.thumb img{display:block;width:auto;height:auto;max-width:100%;max-height:100%;object-fit:contain;object-position:center}.placeholder{font-size:1.05rem;font-weight:900;letter-spacing:-.03em;color:var(--red)}.card .top{display:flex;align-items:center;justify-content:space-between;gap:10px}.badge{font-size:.72rem;color:var(--cyan);background:#26c6e817;padding:5px 8px;border-radius:999px}.actions{display:flex;align-items:center;gap:8px}.stock{font-size:.72rem;font-weight:850;padding:5px 8px;border-radius:999px;white-space:nowrap}.stock-in{color:#9bf5bb;background:#168a4528}.stock-out{color:#fff;background:var(--red)}.stock-unknown{color:var(--muted);background:#a8bdd218}.watch{color:#ffca45;font-size:.78rem;font-weight:800}.favorite{width:38px;height:38px;border:1px solid var(--line);border-radius:50%;background:#07182f99;color:var(--muted);cursor:pointer;font-size:1.2rem;line-height:1}.favorite.saved{color:#ff6680;border-color:#ff6680;background:#e3193720}.card h3{font-size:1rem;line-height:1.5;margin:16px 0;flex:1}.price-row{display:flex;align-items:baseline;gap:10px;flex-wrap:wrap}.price{font-size:1.35rem;font-weight:850;color:#ff7187}.original-price{color:var(--muted);text-decoration:line-through;font-size:.9rem}.discount-rate{color:#fff;background:#e31937;border-radius:999px;padding:3px 8px;font-size:.78rem;font-weight:850;white-space:nowrap}.saving{color:#ffca45;font-size:.78rem;font-weight:800;margin-top:5px}.promotion{color:var(--cyan);font-size:.78rem;margin-top:5px}.promotion a{color:inherit;text-decoration:underline}.unavailable{color:var(--muted);font-size:.78rem;margin-top:5px}.go{color:var(--muted);font-size:.78rem;margin-top:8px}.empty{text-align:center;color:var(--muted);padding:64px 20px;border:1px dashed var(--line);border-radius:20px;grid-column:1/-1}
    ...
    @media(max-width:560px){
      .shell{padding:14px 16px 40px}
      header{padding:18px 0 12px}
      .stats{grid-template-columns:1fr 1fr;gap:8px;margin:16px 0}
      .stat{padding:13px;border-radius:14px}.stat strong{font-size:1.35rem}
      .toolbar{position:static;padding:8px 0 4px}
      .search{border-radius:14px;padding:0 13px}.search input{height:48px;font-size:.95rem}
      .chips{gap:7px;padding:10px 0 4px}.chip{padding:8px 12px;font-size:.9rem}
      .changes{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;margin:14px 0}
      .change{padding:10px 8px;border-radius:13px}.change strong{font-size:1.2rem}.change span{font-size:.7rem}
      .grid{grid-template-columns:1fr;gap:12px}
      .card{min-height:0;padding:10px 12px 15px;border-radius:17px}
      .thumb{aspect-ratio:4/3;max-height:270px;margin-bottom:11px;border-radius:12px}
      .favorite{width:34px;height:34px}
      .card h3{font-size:.98rem;line-height:1.42;margin:12px 0}
      .price{font-size:1.22rem}.original-price{font-size:.84rem}
      .sectionhead{align-items:start;flex-direction:column;gap:10px;margin:22px 0 12px}
      .list-controls{width:100%;justify-content:flex-start}
      .control{flex:1}.control select,.control input{width:100%}
    }
  </style>
</head>
<body>
  <main class="shell">
    <header>
      ...
      <div class="stats">
        ...
        <div class="stat"><strong id="out-of-stock">0</strong><span>目前缺貨</span></div>
    ...
    <div class="changes" id="changes">
      ...
      <button class="change" data-change="未收錄"><strong>${changes.removed.length}</strong><span>本次未收錄</span></button>
    </div>
    <div class="sectionhead">
      ...
      <div class="list-controls">
        ...
        <label class="control">排序
          <select id="sort">
            <option value="relevance">官網相關商品</option>
  ...
  <script>
    ...
    const categories=["全部","缺貨","我的收藏","Apple 追蹤",...new Set(deals.map(d=>d.category))];
    let selected="全部";
    ...
    const stockOf=d=>d.stockStatus||d.stock_status||"狀態未知";
    document.querySelector("#apple").textContent=deals.filter(isWatched).length;
    document.querySelector("#out-of-stock").textContent=deals.filter(d=>stockOf(d)==="缺貨").length;
    ...
    function moneyNumber(value){const match=String(value||"").replaceAll(",","").match(/[0-9]+/);return match?Number(match[0]):NaN}
    function discountRate(original,price,discount){const originalNumber=moneyNumber(original),saleNumber=moneyNumber(price);if(!discount||!Number.isFinite(originalNumber)||!Number.isFinite(saleNumber)||saleNumber<=0||saleNumber>=originalNumber)return "";const rate=Math.round(saleNumber/originalNumber*100)/10;return (Number.isInteger(rate)?String(rate):rate.toFixed(1))+"折"}
    function renderCard(d){const priceNote=d.old_price?'<div class="go">'+escapeHtml(d.old_price)+' → '+escapeHtml(d.price)+'（'+escapeHtml(d.direction)+'）</div>':'';const original=d.originalPrice||d.original_price||"",discount=d.discountAmount||d.discount_amount||"",promotion=d.promotion||"",stock=stockOf(d),stockClass=stock==="缺貨"?"stock-out":stock==="有貨"?"stock-in":"stock-unknown",rate=discountRate(original,d.price,discount),isFurnitureCampaign=promotion.includes("2026/07/20-08/02期間購買指定家具"),isConditionalCampaign=isFurnitureCampaign||promotion.includes("指定夏日涼感商品"),hasDiscountDetails=Boolean(original||discount||promotion);const promotionHtml=isFurnitureCampaign?'<div class="promotion">2026/07/20-08/02期間<a href="https://www.costco.com.tw/c/Hero_FurnitureSale" target="_blank" rel="noopener">購買指定家具</a>，買1件享85折，買2件享8折優惠。 <a href="https://www.costco.com.tw/voucher2" target="_blank" rel="noopener">&lt;&lt;優惠券注意事項&gt;&gt;</a></div>':(promotion?'<div class="promotion">'+escapeHtml(promotion)+'</div>':'');const saved=favorites.has(d.url);return '<article class="card '+(stock==="缺貨"?'out-of-stock':'')+'"><a class="product-link" href="'+d.url+'" target="_blank" rel="noopener"><div class="thumb">'+(d.image?'<img src="'+escapeHtml(d.image)+'" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span class="placeholder" hidden>Costco 優惠</span>':'<span class="placeholder">Costco 優惠</span>')+'</div></a><div class="top"><span class="badge">'+escapeHtml(d.category)+'</span><span class="actions"><span class="stock '+stockClass+'">'+escapeHtml(stock)+"</span>"+(isWatched(d)?'<span class="watch">★ 追蹤</span>':'')+'<button class="favorite '+(saved?'saved':'')+'" data-url="'+escapeHtml(d.url)+'" aria-label="'+(saved?'取消收藏':'加入收藏')+'" title="'+(saved?'取消收藏':'加入收藏')+'">'+(saved?'♥':'♡')+'</button></span></div><h3><a class="product-link" href="'+d.url+'" target="_blank" rel="noopener">'+escapeHtml(d.name)+'</a></h3><div class="price-row">'+(original?'<span class="original-price">原價 '+escapeHtml(original)+'</span>':'')+'<span class="price">'+(isConditionalCampaign&&!original?'價格 ':(original?'特價 ':'優惠售價 '))+escapeHtml(d.price)+'</span>'+(rate?'<span class="discount-rate">'+rate+'</span>':'')+'</div>'+(discount?'<div class="saving">省下 '+escapeHtml(discount)+'</div>':'')+promotionHtml+(!hasDiscountDetails?'<div class="unavailable">Costco 未公開原價與折扣金額</div>':'')+priceNote+'<a class="product-link go" href="'+d.url+'" target="_blank" rel="noopener">查看官方商品 →</a></article>'}
    function priceNumber(d){const price=moneyNumber(d.price);return Number.isFinite(price)?price:Number.MAX_SAFE_INTEGER}
    ...
    function render(){const q=search.value.trim().toLowerCase();let source=deals;if(selected==="新增")source=dailyChanges.added;if(selected==="價格變動")source=dailyChanges.price_changes;if(selected==="未收錄")source=dailyChanges.removed;const special=["新增","價格變動","未收錄"].includes(selected);const filtered=sortDeals(source.filter(d=>(special||selected==="全部"||(selected==="缺貨"?stockOf(d)==="缺貨":selected==="我的收藏"?favorites.has(d.url):selected==="Apple 追蹤"?isWatched(d):d.category===selected))&&(!q||d.name.toLowerCase().includes(q))&&matchesPrice(d)));heading.textContent=selected==="未收錄"?"本次未收錄":(special?"今日"+selected:selected);result.textContent=selected==="未收錄"?"本次抓取未收錄 "+filtered.length+" 項；不代表優惠已結束，請以 Costco 官網為準。":'顯示 '+filtered.length+' 項商品';grid.innerHTML=filtered.length?filtered.map(renderCard).join(""):'<div class="empty">'+(selected==="我的收藏"?"尚未收藏商品，請按商品旁的 ♡。":"沒有符合目前條件的商品。")+'</div>';bindFavorites()}
