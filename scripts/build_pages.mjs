...
<body>
  ...
  <script>
    ...
    let selected="全部";
    const search=document.querySelector("#search"),sort=document.querySelector("#sort"),grid=document.querySelector("#grid"),chips=document.querySelector("#chips"),result=document.querySelector("#result"),heading=document.querySelector("#heading"),changeButtons=document.querySelectorAll(".change");
    ...
    function render(){const q=search.value.trim().toLowerCase();let source=deals;if(selected==="新增")source=dailyChanges.added;if(selected==="價格變動")source=dailyChanges.price_changes;if(selected==="已結束")source=dailyChanges.removed;const special=["新增","價格變動","已結束"].includes(selected);const filtered=sortDeals(source.filter(d=>(special||selected==="全部"||(selected==="我的收藏"?favorites.has(d.url):selected==="Apple 追蹤"?isWatched(d):d.category===selected))&&(!q||d.name.toLowerCase().includes(q))));heading.textContent=special?"今日"+selected:selected;result.textContent='顯示 '+filtered.length+' 項商品';grid.innerHTML=filtered.length?filtered.map(renderCard).join(""):'<div class="empty">'+(selected==="我的收藏"?"尚未收藏商品，請按商品旁的 ♡。":"今天沒有這類變動。")+'</div>';bindFavorites()}
    ...
    search.addEventListener("input",render);sort.addEventListener("change",render);renderChips();render();
  </script>
</body>
</html>`;
